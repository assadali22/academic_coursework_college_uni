...
public:
	...
	//Overloaded Operators (comparing height)
	bool operator==(const personType& other) const;
	bool operator!=(const personType& other) const;
	bool operator>(const personType& other) const;
	bool operator<(const personType& other) const;
	//If the functions aren't defined in cpp file, nothing will change.

	//overload << and >>, friend functions, non member
	friend ostream& operator<<(ostream&, const personType &); //pretty much ready
	friend istream& operator>>(istream&, personType &); //pretty much ready
