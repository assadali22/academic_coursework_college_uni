...
professorType::professorType(string first, string last):personType(first, last)
{
    setEmpID("EMPTY");
    setDepartment("EMPTY");
    setDegree("EMPTY");
...
:personType(first, last, address, height, dob, gender)
{
    setEmpID(empID);
    setDepartment(dep);
    setDegree(deg);
...
/*
The code below doesn't need to be inside the constructor
...
is the same thing, which passes the appropriate data into the uber constructor
of the personType class.
*/