...
bool RedBlackTree::insert(int key) {
  ...
  traverse(root); //assign rank (https://github.com/imhelendang/Red-Black-Tree/blob/main/RedBlackTree.cpp#L98)
  return true;
}
...
size_t RedBlackTree::rankOf(int key) const {
  ...
  //(void) key;
	...
	size_t rank;
	rank = inorderWalkForRank(root, key);
  	return rank;
}
...
int RedBlackTree::select(size_t rank) const {
  ...
  //(void) rank;
	...
	if (root->count <= rank)
	{
		throw runtime_error("Error, rank should be less than the number of elements.");
	}

 	int num = -1;
	Node* location = selectHelper(root, rank, num); //find the node with corresponding rank, track it
...
void RedBlackTree::printDebugInfoRec(Node* root, unsigned indent) const {
  if (root == nullptr) {
    ...
    cout << setw(indent) << "" << "Color:     " << colorToString(root->color) << '\n';
	 cout << setw(indent) << "" << "Count of nodes:     " << root->count << '\n';
...
//Reference: http://www.cs.yale.edu/homes/aspnes/pinewiki/OrderStatisticsTree.html
size_t RedBlackTree::inorderWalkForRank(const struct Node* root, int key) const
{
	if (root == NULL)
	{
		return 0;
	...
	{
		return inorderWalkForRank(root->left, key);
	}
	else if (key > root->key)
	{
		if (root->left != NULL)
		{
			return (1 + root->left->count + inorderWalkForRank(root->right, key));
		}
		else
		{
			return 1 + inorderWalkForRank(root->right, key);
		}
	}
	else if (key == root->key)
	{
...
//Reference: https://github.com/imhelendang/Red-Black-Tree/blob/main/RedBlackTree.cpp#L412
RedBlackTree::Node* RedBlackTree::selectHelper(struct Node* node, int rank, int& num) const
{
	if (node == NULL)
	{
		return NULL;
	...
	Node* left = selectHelper(node->left, rank, num);
	...
	return selectHelper(node->right, rank, num); //check to the right of the node
}



//Reference: https://github.com/imhelendang/Red-Black-Tree/blob/main/RedBlackTree.cpp#L412
int RedBlackTree::countNodes(Node* n) const
{
	if (n == NULL) return 0;
...
//Reference: https://github.com/imhelendang/Red-Black-Tree/blob/main/RedBlackTree.cpp#L412
void RedBlackTree::traverse(Node* n) const
{
	if (n == NULL)
	{
		return;
	...
	traverse(n->left);
	n->count = countNodes(n); //assign count
	traverse(n->right);
}
